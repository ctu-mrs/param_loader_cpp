...
namespace param_loader_cpp
{
   ...
   */
  template <typename T>
  std::ostream& operator<<(std::ostream& os, const MatrixX<T>& var);
...
private:
  ...
  param_loader_cpp::ParamProvider              m_pp;
...
//}
  ...
  // loadMatrixX helper function for loading dynamic Eigen matrices //{
  template <typename T>
  std::pair<MatrixX<T>, bool> ParamLoader::loadMatrixX(const std::string& name, const MatrixX<T>& default_value, int rows, int cols, optional_t optional, unique_t unique, swap_t swap, bool printValues)
  {
    const auto resolved_name = resolved_name_t(name);
  ...
  /* loadMatrixKnown_internal helper function for loading EigenXd matrices with known dimensions //{ */
  template <typename T>
  std::pair<MatrixX<T>, bool> ParamLoader::loadMatrixKnown_internal(const std::string& name, const MatrixX<T>& default_value, int rows, int cols, optional_t optional, unique_t unique)
  {
    MatrixX<T> loaded = default_value;
    // first, check that at least one dimension is set
    if (rows <= 0 || cols <= 0) {
      printError(std::string("Invalid expected matrix dimensions for parameter ") + resolved_name_t(name).str + std::string(" (use loadMatrixDynamic?)"));
      m_load_successful = false;
      return {loaded, false};
    }
  ...
  /* loadMatrixDynamic_internal helper function for loading Eigen matrices with one dynamic (unspecified) dimension //{ */
  template <typename T>
  std::pair<MatrixX<T>, bool> ParamLoader::loadMatrixDynamic_internal(const std::string& name, const MatrixX<T>& default_value, int rows, int cols, optional_t optional, unique_t unique)
  {
    MatrixX<T> loaded = default_value;

    // next, check that at least one dimension is set
    if (rows <= 0 && cols <= 0) {
      printError(std::string("Invalid expected matrix dimensions for parameter ") + resolved_name_t(name).str +
  ...
  /* loadMatrixArray_internal helper function for loading an array of EigenXd matrices with known dimensions //{ */
  template <typename T>
  std::vector<MatrixX<T>> ParamLoader::loadMatrixArray_internal(const std::string& name, const std::vector<MatrixX<T>>& default_value, optional_t optional, unique_t unique)
  {
    const auto resolved_name = resolved_name_t(name);
    int rows;
   ...
   * \brief Loads a compulsory parameter from the rosparam server.
   *
   * If the parameter with the specified name is not focnd on the rosparam server (e.g. because it is not specified in the launchfile or yaml config file),
   * the loading process is unsuccessful (loaded_successfully() will return false).
   * Using this method, the parameter can only be loaded once using the same ParamLoader instance without error.
   *
   * \param name          Name of the parameter in the rosparam server.
   ...
   */
  template <typename T>
  MatrixX<T> ParamLoader::loadParam2(const std::string& name, const MatrixX<T>& default_value)
  {
    MatrixX<T> ret;
   ...
   */
  template <typename T>
  MatrixX<T> ParamLoader::loadMatrixKnown2(const std::string& name, int rows, int cols)
  {
    MatrixX<T> ret;
   ...
   */
  template <typename T, typename Derived>
  MatrixX<T> ParamLoader::loadMatrixKnown2(const std::string& name, const Eigen::MatrixBase<Derived>& default_value, int rows, int cols)
  {
    MatrixX<T> ret;
   ...
   */
  template <typename T>
  MatrixX<T> ParamLoader::loadMatrixDynamic2(const std::string& name, int rows, int cols)
  {
    MatrixX<T> ret;
   ...
   */
  template <typename T, typename Derived>
  MatrixX<T> ParamLoader::loadMatrixDynamic2(const std::string& name, const Eigen::MatrixBase<Derived>& default_value, int rows, int cols)
  {
    MatrixX<T> ret;
   ...
   */
  template <typename T>
  std::vector<MatrixX<T>> ParamLoader::loadMatrixArray2(const std::string& name)
  {
   ...
   */
  template <typename T>
  std::vector<MatrixX<T>> ParamLoader::loadMatrixArray2(const std::string& name, const std::vector<MatrixX<T>>& default_value)
  {
